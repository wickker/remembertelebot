-- name: CreateJob :one
INSERT INTO jobs (telegram_chat_id, is_recurring, message, schedule, name, river_job_id, asynq_job_id)
...
-- name: DeleteScheduledJobByAsynqJobID :one
UPDATE jobs
SET deleted_at = NOW()
WHERE asynq_job_id = $1
AND is_recurring = false
RETURNING *;
